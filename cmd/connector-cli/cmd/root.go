...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.vpn-rotator.yaml)")

	// Add persistent verbose flag for logging (available to all subcommands)
	rootCmd.PersistentFlags().BoolP("verbose", "v", false, "Enable verbose logging")

	// Bind persistent flags to viper so they are available via env/config/flags
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".vpn-rotator" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigType("yaml")
		viper.SetConfigName(".vpn-rotator")
	}

	// Allow env vars to be read in a consistent way, mapping dashes/dots to underscores
